...
def generate_filename(instance, filename):
    ...
    if instance.filename.split('.')[-1] in video_extension:
        return "uploads/video/{file}".format(file=str(instance.id))
    if instance.filename.split('.')[-1] in image_extension:
        return "uploads/image/{file}".format(file=str(instance.id))
    if instance.filename.split('.')[-1] in gif_extension:
        return "uploads/gif/{file}".format(file=str(instance.id))
...
class AbstractChunkedUpload(models.Model):
    ...
    file = models.FileField(max_length=255, upload_to=generate_filename, storage=_settings.STORAGE, null=True)
